        ...
        if (!FileSystem.exists(Main.path + 'presets/marathon/current'))
            {
                ...
                File.saveContent((Main.path + 'presets/marathon/current'), Json.stringify(_marathon));
            }
        else
            {
    ...
    public static function savePreset(input:String):Void
        {
            ...
            File.saveContent((Main.path + 'presets/marathon/'+input), Json.stringify(_marathon)); //just an example for now
        }
}
